...
#pragma omp parallel
  {
    int l = omp_get_thread_num() * (ARRAY_SIZE + omp_get_num_threads() - 1) / omp_get_num_threads();
    int r = (omp_get_thread_num() + 1) * (ARRAY_SIZE + omp_get_num_threads() - 1) / omp_get_num_threads() < ARRAY_SIZE ? (omp_get_thread_num() + 1) * (ARRAY_SIZE + omp_get_num_threads() - 1) / omp_get_num_threads() : ARRAY_SIZE;
    for (int i = l; i < r; i++)
      z[i] = x[i] + y[i];
  }
}
...
#pragma omp for
    for (int i = 0; i < arr_size; i++) {
      local_sum += x[i] * y[i];                    // 每个线程独立累加
    }

    // 使用临界区或 atomic 来保证线程安全地更新 global_sum
#pragma omp critical
    {
